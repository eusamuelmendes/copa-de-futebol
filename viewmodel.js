...
(function (global) {
  ...
  var BUCKET_STYLE = {
    live: {
      centerFg: "var(--ink-solid)", subFg: "#FF6B75", chipFg: "#FF6B75",
      cardBg: "linear-gradient(120deg,rgba(30,107,255,.22),rgba(11,27,60,.85) 55%,rgba(255,46,63,.12))",
      cardBd: "rgba(63,169,255,.42)", cta: "ACOMPANHAR →", ctaFg: "var(--ink-solid)"
    },
    done: {
      centerFg: "rgba(var(--ink-rgb),.88)", subFg: "rgba(var(--ink-rgb),.4)", chipFg: "rgba(var(--ink-rgb),.55)",
      cardBg: "rgba(var(--ink-rgb),.045)", cardBd: "rgba(var(--ink-rgb),.09)", cta: "SÚMULA →", ctaFg: "rgba(var(--ink-rgb),.5)"
    },
    next: {
      centerFg: "rgba(var(--ink-rgb),.34)", subFg: "#8FD0FF", chipFg: "#8FD0FF",
      cardBg: "rgba(var(--ink-rgb),.045)", cardBd: "rgba(var(--ink-rgb),.09)", cta: "LEMBRETE →", ctaFg: "rgba(var(--ink-rgb),.5)"
    }
  };
  ...
  function matchCard(game, data, ui, actions) {
    ...
    return {
      ...
      canShare: canShare,
      share: function (e) { if (e && e.stopPropagation) e.stopPropagation(); actions.shareMatch(game.id); }
    };
  }
  ...
  function buildDetail(game, data, ui, actions) {
    ...
    var tabs = tabDefs.map(function (t) {
      ...
      return {
        ...
        fg: on ? "var(--c-page)" : "rgba(var(--ink-rgb),.6)", bg: on ? "var(--ink-solid)" : "rgba(var(--ink-rgb),.06)"
      };
    });
  ...
  function standingsRowShape(r, idx, cutoff, data, ui, actions) {
    ...
    return {
      ...
      apr: pct + "%", aprW: pct + "%",
      zone: classified ? "#3FA9FF" : "rgba(var(--ink-rgb),.18)",
      rowBg: !classified ? "transparent" : (idx === 0 ? "rgba(63,169,255,.12)" : "rgba(63,169,255,.06)"),
      posFg: classified ? "#8FD0FF" : "rgba(var(--ink-rgb),.5)",
  ...
  function buildGroupsTabs(groupsList, activeGroup, actions) {
    return groupsList.map(function (g) {
      ...
      return {
        ...
        fg: on ? "var(--ink-solid)" : "rgba(var(--ink-rgb),.6)",
        bg: on ? "linear-gradient(100deg,#1E6BFF,#3FA9FF)" : "rgba(var(--ink-rgb),.06)",
        bd: on ? "rgba(143,208,255,.6)" : "rgba(var(--ink-rgb),.1)"
      };
    });
  }
  ...
  // ---- página do artilheiro (ranking completo) -------------------------------
  // Jogos "disputados", pra fim de desempate, é a contagem de partidas
  // Encerradas/WO do TIME do artilheiro — a planilha não guarda escalação por
  // jogo, só quem marcou, então esse é o número disponível mais próximo de
  // "quantos jogos ele teve chance de balançar as redes".
  function jogosDisputadosPorTime(data) {
    var acc = {};
    jogosEncerrados(data).forEach(function (g) {
      [g.timeCasaId, g.timeForaId].forEach(function (id) {
        if (!id) return;
        acc[id] = (acc[id] || 0) + 1;
      });
    });
  ...
  function buildAllScorers(data, ui, actions) {
    ...
    var jpt = jogosDisputadosPorTime(data);
    var list = Object.keys(counts)
      .filter(function (id) { return data.jogadorById[id]; })
      .map(function (id) {
        var j = data.jogadorById[id];
        var team = data.timeById[j.timeId];
        var av = playerAvatar(j, ui);
        return {
          name: j.nome, teamId: j.timeId, team: team ? team.nome : "", grupo: team ? team.grupo : "",
          goals: counts[id], jogos: jpt[j.timeId] || 0,
          g: av.g, i: av.i,
          open: team ? (function (tid) { return function () { actions.openTeam(tid); }; })(team.id) : function () {}
        };
      });
    list.sort(function (a, b) {
      if (b.goals !== a.goals) return b.goals - a.goals;
      if (a.jogos !== b.jogos) return a.jogos - b.jogos; // menos jogos disputados desempata pra cima
      return a.name.localeCompare(b.name);
    });
    var out = [];
    var pos = 0, prevGoals = null, prevJogos = null;
    list.forEach(function (s, i) {
      if (s.goals !== prevGoals || s.jogos !== prevJogos) pos = i + 1;
      prevGoals = s.goals; prevJogos = s.jogos;
      out.push({
        pos: pos + "º", i: s.i, g: s.g, name: s.name, team: s.team, teamId: s.teamId,
        grupo: s.grupo, goals: s.goals, jogos: s.jogos,
        jogosLabel: s.jogos + (s.jogos === 1 ? " jogo" : " jogos"),
        open: s.open
      });
    });
    return out;
  ...
  function buildScorersGrupoFilters(data, ui, actions) {
    ...
    return ["Todos"].concat(groupsList).map(function (g) {
      ...
      return {
        label: g === "Todos" ? "Todos" : "GRUPO " + g, fg: c.fg, bg: c.bg, bd: c.bd,
        pick: function () { actions.pickScorersGrupo(g); }
      };
    });
  }
  ...
  function buildViews(ui, actions) {
    return ["Data", "Rodada", "Fase"].map(function (v) {
      var on = v === ui.view;
      return { label: v, fg: on ? "var(--c-page)" : "rgba(var(--ink-rgb),.6)", bg: on ? "var(--ink-solid)" : "transparent", pick: function () { actions.pickView(v); } };
    });
  }
  ...
  function buildTeamsGrouped(data, ui, actions, teamBadges) {
    var groupsList = buildGroupsList(data);
    return groupsList.map(function (g) {
  ...
  function buildTeamGames(data, teamId, ui, actions) {
    return data.jogos.filter(function (g) { return g.timeCasaId === teamId || g.timeForaId === teamId; })
      ...
      .map(function (g) {
        ...
        var h2h = buildHeadToHeadHistory(data, teamId, oppId);
        var h2hSummary = "";
        if (h2h.length) {
          var vitórias = h2h.filter(function (m) { return m.resultado === "V"; }).length;
          var derrotas = h2h.filter(function (m) { return m.resultado === "D"; }).length;
          var empates = h2h.filter(function (m) { return m.resultado === "E"; }).length;
          h2hSummary = vitórias + "V " + empates + "E " + derrotas + "D";
        }
        ...
        return {
          ...
          h2hSummary: h2hSummary, hasH2H: !!h2hSummary,
          open: card.open
        };
      });
  }
  ...
  // ---- time favorito + contagem regressiva ----------------------------------
  // Sem favorito marcado, retorna null e o bloco inteiro some da Home. Com
  // favorito: pega o próximo jogo dele (ao vivo/intervalo entra antes de
  // agendado, senão o mais próximo por data+hora). Ao vivo/intervalo vira o
  // placar em tempo real (reaproveita matchCard); agendado vira contagem
  // regressiva dias/horas/minutos, recalculada a cada render (ui.now).
  function buildFavorito(data, teamId, ui, actions) {
    if (!teamId) return null;
    var team = data.timeById[teamId];
    if (!team) return null;
    var badge = teamBadge(team, ui);
    var stats = buildFavoritoStats(data, teamId);
    var upcoming = data.jogos.filter(function (g) {
      return (g.timeCasaId === teamId || g.timeForaId === teamId) &&
        (g.status === "Ao vivo" || g.status === "Intervalo" || g.status === "Agendado");
    }).sort(function (a, b) {
      // ao vivo/intervalo sempre primeiro; entre os demais, o mais próximo
      var la = (a.status === "Ao vivo" || a.status === "Intervalo") ? "0" : "1";
      var lb = (b.status === "Ao vivo" || b.status === "Intervalo") ? "0" : "1";
      var ka = la + a.data + a.hora, kb = lb + b.data + b.hora;
      return ka.localeCompare(kb);
    });
    var game = upcoming[0];
    var base = { id: team.id, nome: team.nome, i: badge.i, g: badge.g, corHex: team.corHex, hasGame: !!game, stats: stats };
    if (!game) return base;

    var isLiveNow = game.status === "Ao vivo" || game.status === "Intervalo";
    var isHome = game.timeCasaId === teamId;
    var oppId = isHome ? game.timeForaId : game.timeCasaId;
    var opp = data.timeById[oppId] || { nome: "A definir", corHex: FALLBACK_HEX };
    var oppBadge = teamBadge(opp, ui);
    base.isLive = isLiveNow;
    base.card = matchCard(game, data, ui, actions);
    base.oppNome = opp.nome; base.oppI = oppBadge.i; base.oppG = oppBadge.g;
    base.isHome = isHome;
    base.local = game.local || "";
    base.roundLabel = (game.rodada ? "RODADA " + game.rodada : "") + (game.fase ? (game.rodada ? " · " : "") + game.fase : "");

    if (!isLiveNow) {
      base.cdLabel = scheduleLabel(game);
      base.cdMeta = scheduleLabel(game) + (game.local ? " · " + game.local : "");
      var target = combineDateTime(game.data, game.hora);
      base.hasCountdown = false;
      if (target) {
        var diffMs = target.getTime() - ui.now.getTime();
        if (diffMs > 0) {
          var totalMin = Math.floor(diffMs / 60000);
          var days = Math.floor(totalMin / 1440);
          var hours = Math.floor((totalMin % 1440) / 60);
          var mins = totalMin % 60;
          base.hasCountdown = true;
          base.cdDays = pad2(days); base.cdHours = pad2(hours); base.cdMins = pad2(mins);
          base.cdDaysN = days;
        }
      }
    }
  ...
  // ---- Recurso 3: Estatísticas do time favorito ------
  function buildFavoritoStats(data, teamId) {
    if (!teamId) return null;
    var jogos = jogosEncerrados(data).filter(function (g) {
      return g.timeCasaId === teamId || g.timeForaId === teamId;
    });
    if (!jogos.length) return null;
    var stats = { jogos: 0, vitórias: 0, golsMarcados: 0, golsSofridos: 0 };
    jogos.forEach(function (g) {
      var isHome = g.timeCasaId === teamId;
      var golPro = isHome ? g.golsCasa : g.golsFora;
      var golContra = isHome ? g.golsFora : g.golsCasa;
      stats.jogos++;
      stats.golsMarcados += golPro || 0;
      stats.golsSofridos += golContra || 0;
      if ((golPro || 0) > (golContra || 0)) stats.vitórias++;
    });
    return stats;
  }

  // ---- Recurso 6: Leaderboard extra ------
  function buildWeeklyStats(data, ui) {
    var now = ui.now;
    var weekAgo = new Date(now);
    weekAgo.setDate(weekAgo.getDate() - 7);

    var weekJogos = jogosEncerrados(data).filter(function (g) {
      var d = parseYMD(g.data);
      return d >= weekAgo;
    });

    if (!weekJogos.length) return { bestTeam: null, biggestScore: null };

    // Melhor time da semana (maior saldo de gols)
    var timeStats = {};
    weekJogos.forEach(function (g) {
      [[g.timeCasaId, g.golsCasa, g.golsFora], [g.timeForaId, g.golsFora, g.golsCasa]].forEach(function (p) {
        if (!p[0]) return;
        if (!timeStats[p[0]]) timeStats[p[0]] = { id: p[0], saldo: 0 };
        timeStats[p[0]].saldo += (p[1] || 0) - (p[2] || 0);
      });
    });

    var bestTeams = melhores(Object.keys(timeStats).map(function (k) { return timeStats[k]; }), function (t) { return t.saldo; }, true);
    var bestTeam = bestTeams.length ? bestTeams[0] : null;
    if (bestTeam && data.timeById[bestTeam.id]) {
      bestTeam.nome = data.timeById[bestTeam.id].nome;
      bestTeam.badge = teamBadge(data.timeById[bestTeam.id], null);
    }

    // Maior goleada da semana
    var biggestScores = melhores(weekJogos, function (g) { return Math.abs((g.golsCasa || 0) - (g.golsFora || 0)); }, true);
    var biggestScore = biggestScores.length ? biggestScores[0] : null;
    if (biggestScore) {
      var teamA = data.timeById[biggestScore.timeCasaId];
      var teamB = data.timeById[biggestScore.timeForaId];
      biggestScore.timeCasaNome = teamA ? teamA.nome : "Time A";
      biggestScore.timeForaNome = teamB ? teamB.nome : "Time B";
    }

    return { bestTeam: bestTeam, biggestScore: biggestScore };
  }

  // ---- Recurso 7: Ranking de curiosidades ------
  function buildCuriosities(data, ui) {
    var done = jogosEncerrados(data);
    if (!done.length) return [];

    var curiosities = [];

    // 1. Melhor defesa (menos gols sofridos)
    var defensaStats = {};
    done.forEach(function (g) {
      [[g.timeCasaId, g.golsFora], [g.timeForaId, g.golsCasa]].forEach(function (p) {
        if (!p[0]) return;
        if (!defensaStats[p[0]]) defensaStats[p[0]] = { id: p[0], sofrido: 0, jogos: 0 };
        defensaStats[p[0]].sofrido += p[1] || 0;
        defensaStats[p[0]].jogos++;
      });
    });
    var melhorDefesa = Object.keys(defensaStats).map(function (k) { return defensaStats[k]; });
    melhorDefesa = melhores(melhorDefesa, function (t) { return t.sofrido / Math.max(1, t.jogos); }, false);
    if (melhorDefesa.length) {
      var team = data.timeById[melhorDefesa[0].id];
      curiosities.push({
        icon: "🛡️", label: "Melhor Defesa", nome: team ? team.nome : "?",
        valor: melhorDefesa[0].sofrido + " gols em " + melhorDefesa[0].jogos + " j."
      });
    }

    // 2. Atacante (mais gols marcados)
    var ataqueStats = {};
    done.forEach(function (g) {
      [[g.timeCasaId, g.golsCasa], [g.timeForaId, g.golsFora]].forEach(function (p) {
        if (!p[0]) return;
        if (!ataqueStats[p[0]]) ataqueStats[p[0]] = { id: p[0], marcado: 0, jogos: 0 };
        ataqueStats[p[0]].marcado += p[1] || 0;
        ataqueStats[p[0]].jogos++;
      });
    });
    var melhorAtaque = Object.keys(ataqueStats).map(function (k) { return ataqueStats[k]; });
    melhorAtaque = melhores(melhorAtaque, function (t) { return t.marcado; }, true);
    if (melhorAtaque.length) {
      var team = data.timeById[melhorAtaque[0].id];
      curiosities.push({
        icon: "⚡", label: "Melhor Ataque", nome: team ? team.nome : "?",
        valor: melhorAtaque[0].marcado + " gols em " + melhorAtaque[0].jogos + " j."
      });
    }

    // 3. Melhor aproveitamento (gols marcados / gols sofridos)
    var aproveitStats = {};
    done.forEach(function (g) {
      [[g.timeCasaId, g.golsCasa, g.golsFora], [g.timeForaId, g.golsFora, g.golsCasa]].forEach(function (p) {
        if (!p[0]) return;
        if (!aproveitStats[p[0]]) aproveitStats[p[0]] = { id: p[0], pro: 0, contra: 0 };
        aproveitStats[p[0]].pro += p[1] || 0;
        aproveitStats[p[0]].contra += p[2] || 0;
      });
    });
    var melhorAproveit = Object.keys(aproveitStats).map(function (k) {
      var a = aproveitStats[k];
      return { id: a.id, ratio: a.pro / Math.max(1, a.contra) };
    });
    melhorAproveit = melhores(melhorAproveit, function (t) { return t.ratio; }, true);
    if (melhorAproveit.length) {
      var team = data.timeById[melhorAproveit[0].id];
      curiosities.push({
        icon: "🎯", label: "Melhor Aproveitamento", nome: team ? team.nome : "?",
        valor: "Índice " + (melhorAproveit[0].ratio).toFixed(2)
      });
    }

    // 4. Sequência invicta (times que não perderam)
    var unbeaten = data.times.filter(function (t) {
      var timesJogos = done.filter(function (g) { return g.timeCasaId === t.id || g.timeForaId === t.id; });
      return timesJogos.length > 0 && timesJogos.every(function (g) {
        var isHome = g.timeCasaId === t.id;
        var pro = isHome ? g.golsCasa : g.golsFora;
        var contra = isHome ? g.golsFora : g.golsCasa;
        return pro >= contra;
      });
    });
    if (unbeaten.length) {
      curiosities.push({
        icon: "🔥", label: "Invictos", nome: unbeaten.map(function (t) { return t.nome; }).slice(0, 2).join(" / "),
        valor: unbeaten.length + " time(s)"
      });
    }

    return curiosities.slice(0, 4);
  }

  // ---- Recurso 8: Badges para times ------
  function buildTeamBadges(data) {
    var done = jogosEncerrados(data);
    if (!done.length) return {};
    ...
    var defensaStats = {};
      ...
      [[g.timeCasaId, g.golsFora], [g.timeForaId, g.golsCasa]].forEach(function (p) {
        if (!p[0]) return;
        if (!defensaStats[p[0]]) defensaStats[p[0]] = { id: p[0], sofrido: 0 };
        defensaStats[p[0]].sofrido += p[1] || 0;
      });
    });
    var melhorDefesa = melhores(Object.keys(defensaStats).map(function (k) { return defensaStats[k]; }), function (t) { return t.sofrido; }, false);
    if (melhorDefesa.length) {
      badges[melhorDefesa[0].id] = (badges[melhorDefesa[0].id] || []).concat("🛡️");
    }
    ...
    var ataqueStats = {};
      ...
      [[g.timeCasaId, g.golsCasa], [g.timeForaId, g.golsFora]].forEach(function (p) {
        if (!p[0]) return;
        if (!ataqueStats[p[0]]) ataqueStats[p[0]] = { id: p[0], marcado: 0 };
        ataqueStats[p[0]].marcado += p[1] || 0;
      });
    });
    var melhorAtaque = melhores(Object.keys(ataqueStats).map(function (k) { return ataqueStats[k]; }), function (t) { return t.marcado; }, true);
    if (melhorAtaque.length) {
      badges[melhorAtaque[0].id] = (badges[melhorAtaque[0].id] || []).concat("⚡");
    }

    // Invictos
    data.times.forEach(function (t) {
      var timesJogos = done.filter(function (g) { return g.timeCasaId === t.id || g.timeForaId === t.id; });
      if (timesJogos.length > 0 && timesJogos.every(function (g) {
        var isHome = g.timeCasaId === t.id;
        var pro = isHome ? g.golsCasa : g.golsFora;
        var contra = isHome ? g.golsFora : g.golsCasa;
        return pro >= contra;
      })) {
        badges[t.id] = (badges[t.id] || []).concat("🔥");
      }
    });
  ...
  // ---- Recurso 9: Histórico de confrontos diretos ------
  function buildHeadToHeadHistory(data, team1Id, team2Id) {
    if (!team1Id || !team2Id) return [];
    var done = jogosEncerrados(data);
    var h2h = done.filter(function (g) {
      return (g.timeCasaId === team1Id && g.timeForaId === team2Id) ||
             (g.timeCasaId === team2Id && g.timeForaId === team1Id);
    }).slice(-3);

    return h2h.map(function (g) {
      var isHome1 = g.timeCasaId === team1Id;
      var pro1 = isHome1 ? g.golsCasa : g.golsFora;
      var contra1 = isHome1 ? g.golsFora : g.golsCasa;
      var resultado = pro1 > contra1 ? "V" : (pro1 < contra1 ? "D" : "E");
      return {
        data: formatDatePt(g.data), placar: pro1 + "×" + contra1, resultado: resultado,
        local: isHome1 ? "Casa" : "Fora"
      };
    });
  }
  ...
  function buildFollowTeams(data, followedTeams, ui, actions) {
    return data.times.slice().sort(function (a, b) { return a.nome.localeCompare(b.nome); }).map(function (t) {
      ...
      return {
        id: t.id, nome: t.nome, i: teamBadge(t, ui).i, g: teamBadge(t, ui).g,
        on: on, onFg: on ? "var(--ink-solid)" : "rgba(var(--ink-rgb),.6)",
        onBg: on ? "linear-gradient(100deg,#1E6BFF,#3FA9FF)" : "rgba(var(--ink-rgb),.06)",
        onBd: on ? "rgba(143,208,255,.6)" : "rgba(var(--ink-rgb),.1)",
  ...
  function buildVals(data, ui, actions) {
    ...
    var allScorersFull = buildAllScorers(data, ui, actions);
    var scorersGrupoFilter = ui.scorersGrupo && ui.scorersGrupo !== "Todos" ? ui.scorersGrupo : null;
    var allScorers = scorersGrupoFilter ? allScorersFull.filter(function (s) { return s.grupo === scorersGrupoFilter; }) : allScorersFull;
    var favorito = buildFavorito(data, ui.favTeamId, ui, actions);
    var weeklyStats = buildWeeklyStats(data, ui);
    var curiosities = buildCuriosities(data, ui);
    var teamBadges = buildTeamBadges(data);
    ...
    return {
      ...
      teamsGrouped: buildTeamsGrouped(data, ui, actions, teamBadges), hasTeams: data.times.length > 0,
      ...
      favorito: favorito, hasFavorito: !!(favorito && favorito.hasGame),
      weeklyStats: weeklyStats, hasBestTeamOfWeek: !!weeklyStats.bestTeam, hasBiggestScoreOfWeek: !!weeklyStats.biggestScore,
      curiosities: curiosities, hasCuriosities: curiosities.length > 0,
